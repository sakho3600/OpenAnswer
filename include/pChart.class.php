 ...
 class pChart
  ...
  function reportWarnings($Interface="CLI")
   ...
   function setFontProperties($FontName,$FontSize)
	...
	function setXLabelFontSize($FontSize){
		$this->XFONT_SIZE = $FontSize;
	}
       ...
       if ( $ID % $SkipLabels == 0 )
        {
         ...
         $Position   = imageftbbox(($this->XFONT_SIZE != 0?$this->XFONT_SIZE:$this->FontSize),$Angle,$this->FontName,$Value);
         ...
         if ( $Angle == 0 )
          {
           ...
           imagettftext($this->Picture,($this->XFONT_SIZE != 0?$this->XFONT_SIZE:$this->FontSize),$Angle,floor($XPos)-floor($TextWidth/2),$YPos,$C_TextColor,$this->FontName,$Value);
          }
         else
          {
           $YPos = $this->GArea_Y2+10+$TextHeight;
           if ( $Angle <= 90 )
            imagettftext($this->Picture,($this->XFONT_SIZE != 0?$this->XFONT_SIZE:$this->FontSize),$Angle,floor($XPos)-$TextWidth+5,$YPos,$C_TextColor,$this->FontName,$Value);
           else
            imagettftext($this->Picture,($this->XFONT_SIZE != 0?$this->XFONT_SIZE:$this->FontSize),$Angle,floor($XPos)+$TextWidth+5,$YPos,$C_TextColor,$this->FontName,$Value);
          }
     ...
     foreach($Series as $Key => $Serie)
      {
       ...
       foreach ( $Data as $Key => $Values )
        {
         if ( isset($Data[$Key][$Serie]) && is_numeric($Data[$Key][$Serie]) && $Data[$Key][$Serie]>0)
          {
           $Value = round($Data[$Key][$Serie],2);
           $YPos = $this->GArea_Y2 - (($Value-$this->VMin) * $this->DivisionRatio);
   ...
   function Render($FileName)
    {
     ...
     imagepng($this->Picture,$FileName,0);
    }
   ...
   function Stroke()
    {
     ...
     header('Content-type: image/png');
 	 imagepng($this->Picture,NULL,0);
	 //imagejpeg($this->Picture,NULL,100);
