 ...
 * Code Modified by Open Teleservics Corp to update class to PHP 5+ compliant.
 */
...
class Histogram {
	...
	var $N,$MIN,$MAX,$AVG,$STDV,$SUM,$SUM2,$NBINS,$delta;
	var $BINS = array();
	var $BINGRAPH = array();
	var $STATS = array();
	...
	function __construct($data="", $nbins=0, $title="") {
		if ($data && $nbins) {
	...
	function create($data,$nbins,$title="") {
		...
		/* Check if we got a valid set of data and bins */
		 $data_test = ($this->N = count($data)) > 1 ? NULL : "Not enough data, number of values: ".$this->N."\n";
		 $bin_test = ($this->NBINS = $nbins) > 1 ? NULL : "Insuficient number of bins\n";
		 if($data_test != NULL){ return $data_test; }
		 if($bin_test != NULL){ return $bin_test; }
		
		/* initialize values */
		$this->MIN = (float) min($data);
		$this->MAX = (float) max($data);
		$this->delta = (float) ($this->MAX - $this->MIN)/$this->NBINS;
		$this->setTitle($title);
		
		/* init bins array */
		for ($i=0; $i < $this->NBINS; $i++) {
			$bin[$i] = (float) $this->MIN + $this->delta * $i;
	...
	function printBins() {
		...
		reset($this->BINS); $i=1;
		foreach($this->BINS as $key => $val){
			echo sprintf("%d\t%-8.4f\t%-8d |%s\n",$i++,$key,$val,$this->_printBar($val,$maxbin));
		}
	}
