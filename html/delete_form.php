...
elseif(isset($_GET['list']) && $_GET['list']==1 && !isset($_GET['q'])){
	echo display::build_confirm_box("Are you sure you want to <span style=\"font-weight:bold;color:red\">PERMANENTLY DELETE</span> the form <span id=\"fName\"></span>?<br /><span style=\"font-weight:bold;\">THIS ACTION CANNOT BE UN-DONE!</span>", "Confirm Deletion");
...
if(isset($_GET['dlt']) && $_GET['dlt']==1 && is_array($info)){
	$obj = new client();
	$action = $obj->delete_form($_SESSION['user_id'], $_GET['formID']);
